...
namespace Hai.ConstraintTrackAnimationCreator.Scripts.Editor.EmbeddedCtacAac.Fluent
{
    ...
    public class AacFlIntParameter : AacFlParameter
    {
        ...
        private AacFlIntParameter(string name) : base(name) { }
...
/// </ul>
        public IAacFlCondition AreFalseExcept(AacFlBoolParameter exceptThisMustBeTrue) => Just(condition =>
        {
            foreach (var name in _names.Where(name => name != exceptThisMustBeTrue.Name))
            {
                condition.Add(name, IfNot, 0);
            }
            condition.Add(exceptThisMustBeTrue.Name, If, 0);
        });
    ...
    class AacFlBoolParameterIsAnyOrCondition : IAacFlOrCondition
    {
        private readonly string[] _names;
